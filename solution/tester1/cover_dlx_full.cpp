...
int main() {
    ...
    auto check = [&](int x, int y) {
        return !(x < 0 || y < 0 || x >= 5 * n || y >= 5 * m || current[x][y] != current_cnt); 
    };
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j + 1 < m; j += 2) {
            if (n % 2 == 1 && i == n - 1 && j + 2 == m - 1) {
                ++total;
                solver({hblock(i, j), hblock(i, j + 1), hblock(i, j + 2)});
            }
            else {
                ++total;
                solver({hblock(i, j), hblock(i, j + 1)});
            }
        }
        if (m % 2 == 1 && i % 2 == 0 && i + 1 < n)
            ++total, solver({hblock(i, m - 1), hblock(i + 1, m - 1)});
        if (m % 2 == 1 && m == 1 && i + 1 == n)
            ++total, solver({hblock(n - 1, m - 1)});
    }
    for (int i = 0; i < n * 5; ++i)
        for (int j = 0; j < m * 5; ++j)
            std::cout << grid[i][j] << " \n"[j + 1 == 5 * m];
